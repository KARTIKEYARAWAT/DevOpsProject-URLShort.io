...
export default function SplitText({
    ...
    return (
        <div
            className={cn(
                "relative cursor-pointer text-4xl font-black uppercase md:text-5xl",
                className,
            )}
        >
            {/** add hidden text so that we maintain the size for any text */}
            <div className="invisible leading-none text-transparent opacity-0 select-none">{text}</div>
            <div className="absolute top-0 flex h-full w-full justify-start">
                {text.split("").map((letter, index) => (
                    <div
                        ...
                        key={index}
                        className="relative inline-flex h-full cursor-pointer flex-col leading-none"
                    >
                        {/** top half */}
                        <span
